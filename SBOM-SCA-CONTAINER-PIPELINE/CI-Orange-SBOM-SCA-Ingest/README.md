...
# native Trivy JSON - Phoenix infers the asset type (CONTAINER for an image scan)
...
- **Asset typing.** Phoenix's Trivy translator reads the asset type out of the report -
  `container_image` becomes a `CONTAINER` asset. The vulnerability method always builds a
  `BUILD` asset (see [Gotchas](#gotchas)).
