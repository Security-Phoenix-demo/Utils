...
A comprehensive vulnerability import tool for Phoenix Security that supports **205 scanner types** with a hybrid translation system (63 hard-coded translators + YAML fallback), intelligent batching, data validation, and flexible import modes.
...
- **Hybrid Translation System** - 63 hard-coded translators + YAML-based fallback for flexibility
...
[1] phoenix_multi_scanner_enhanced.py (Main Script)
    ↓
[2] Scanner Detection (Hybrid: Hard-coded + YAML)
    ├── 63 Hard-coded Translators (complex scanners)
    └── scanner_field_mapper.py + scanner_field_mappings.yaml (YAML fallback)
    ↓
[3] Data Translation
    ├── Specific Translators (Trivy, Grype, JFrog, Prowler, etc.)
...
Loading_Script_V4/
├── 📄 Main Script
│   └── phoenix_multi_scanner_enhanced.py   # Main entry point (748 lines)
│
├── 📄 Core Import Logic
│   ├── phoenix_multi_scanner_import.py     # Legacy multi-scanner logic
│   ├── phoenix_import_refactored.py        # Core API client (1,897 lines)
│   └── phoenix_import_enhanced.py          # Batching & retry logic (507 lines)
│
├── 📄 Translation Layer (63 Hard-coded Translators)
│   ├── csv_scanners_batch.py               # 7 CSV translators
│   ├── final_three_translators.py          # HackerOne, Wiz Issues, Fortify
│   ├── final_two_translators.py            # MS Defender, ORT
│   ├── last_two_translators.py             # GitLab, TestSSL, GitHub secrets
│   ├── jfrog_xray_translators.py           # 5 JFrog translators
│   ├── prowler_translators.py              # 4 AWS Prowler versions
│   ├── round17_translators.py              # NSP, Snyk CLI
│   ├── round18_final_translators.py        # SonarQube API, AWS Inspector2, Prowler CSV
